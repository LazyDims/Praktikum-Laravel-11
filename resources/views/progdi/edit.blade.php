...
<body>
    <div class="container mt-2">
        <div class="row">
            ...
            @endif
            <form action="{{ route('progdi.update', $progdi->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="mt-8 ml-7">
                    <div class="col-span-4">
                        <label for="name" class="block text-sm leading-6 text-gray-900 font-medium">Nama
                            Fakultas</label>
                        <div class="mt-2">
                            <input name="nm_fakultas" type="text" value="{{ $progdi->nm_fakultas }}"
                ...
                <div class="mt-8 ml-7">
                    <div class="col-span-4">
                        <label for="name" class="block text-sm leading-6 text-gray-900 font-medium">Nama
                            Program Studi</label>
                        <div class="mt-2">
                            <input name="nm_progdi" type="text" value="{{ $progdi->nm_progdi }}"
                                class="block w-5/6  max-h-6 p-4 rounded-md border-0  text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600">
                            @error('nm_progdi')
                                <div class="text-red-600">{{ 'Harap Diisi' }}</div>
                            @enderror
                ...
                <div class="mt-8 ml-7">
                    <button type="submit" class="bg-blue-500 text-white hover:text-black rounded p-2">Update</button>
                </div>
            </form>
        </div>
    </div>
